...
public enum MicroControlMapping {
    ...
    static func input(slot: Int, sessionKeys: [Int], controls: [T3MicroAction: Int],
                      actionButtons: [Int: T3MicroAction] = [:]) -> Input? {
        if let action = fixedActions.first(where: { controls[$0] == slot }) { return .control(action) }
        guard let key = LayerMapping.physicalKey(forAgentSlot: slot) else { return nil }
    ...
    public static func slots(config: [String: Any], target: LayerTarget, sessionKeys: [Int]) throws -> [T3MicroAction: Int] {
        ...
        let sessionSlots = Set(sessionKeys.compactMap(LayerMapping.agentSlot(forPhysicalKey:)))
        ...
        let free = (6...19).filter { !sessionSlots.contains($0) && !occupied.contains($0) }
        guard free.count >= fixedActions.count else {
            throw ConfigurationError.invalid("The dial and joystick down need four free agent slots. Assign at most 10 session and action buttons in total, or free other agent bindings on this layer in Input.")
        }
